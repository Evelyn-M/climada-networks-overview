...
for col in ['TYPE','VOLT_CLASS', 'VOLTAGE','man_made', 'public_tra', 'bus']:
...
cis_network.nodes.to_feather(PATH_SAVE+'/cis_nw_nodes_predeps')
cis_network.edges.to_feather(PATH_SAVE+'/cis_nw_edges_predeps')
...
with open(PATH_SAVE +'/base_stats_FLALGA.pkl', 'wb') as f:
...
tc_michael = TropCyclone.from_hdf5(PATH_DATA+'/tc_michael_1h.hdf5')
tc_michael.check()
...
exp_pnt = u_lp.exp_geom_to_pnt(exp_line, res=500, to_meters=True, 
...
exp_pnt = u_lp.exp_geom_to_pnt(exp_line, res=500, to_meters=True, 
                               disagg_met=u_lp.DisaggMethod.FIX, disagg_val=500)  
