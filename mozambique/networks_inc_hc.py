...
gdf_health_added = gpd.read_feather(PATH_SAVE+'health_sofala_added')
gdf_health = gdf_health.append(gdf_health_added)
...
cis_network.nodes.to_feather(PATH_SAVE+'cis_nw_nodes_m4')
cis_network.edges.to_feather(PATH_SAVE+'cis_nw_edges_m4')

base_stats = nwu.number_noservices(cis_graph,
                         services=['power', 'healthcare', 'education', 
                                   'telecom', 'mobility'])

with open(PATH_SAVE +f'base_stats_{iso3}_m4.pkl', 'wb') as f:
