...
PATH_DEPS = PATH_SAVE + 'dependency_table_MOZ_nohcpow.csv'
...
gdf_health = gpd.read_feather(PATH_HEALTH)
...
cis_network.nodes.to_feather(PATH_SAVE+'cis_nw_nodes_m5')
cis_network.edges.to_feather(PATH_SAVE+'cis_nw_edges_m5')

base_stats = nwu.number_noservices(cis_graph,
                         services=['power', 'healthcare', 'education', 
                                   'telecom', 'mobility'])

with open(PATH_SAVE +f'base_stats_{iso3}_m5.pkl', 'wb') as f:
    pickle.dump(base_stats, f) 

# save country-specific dependency-table.
df_dependencies.to_csv(PATH_SAVE +f'dependency_table_{iso3}_nohcpow.csv')
